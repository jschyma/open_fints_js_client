...
// +998+1+1::0+1:20141216:205751+2:2:13:@8@:5:1+280:12345678:max:V:0:0+0'
      if (me_msg.proto_version == 300) { me_msg.hnvsk = Helper.newSegFromArray('HNVSK', 3, [['PIN', me_msg.sign_it.pin_vers == '999' ? 1 : 2], 998, 1, [1, NULL, me_msg.sign_it.sys_id], [1, Helper.convertDateToDFormat(new Date()), Helper.convertDateToTFormat(new Date())], [2, 2, 13, Helper.Byte('\0\0\0\0\0\0\0\0'), 5, 1], [280, me_msg.sign_it.blz, me_msg.sign_it.kunden_id, 'V', 0, 0], 0]) } else { me_msg.hnvsk = Helper.newSegFromArray('HNVSK', 2, [998, 1, [1, NULL, me_msg.sign_it.sys_id], [1, Helper.convertDateToDFormat(new Date()), Helper.convertDateToTFormat(new Date())], [2, 2, 13, Helper.Byte('\0\0\0\0\0\0\0\0'), 5, 1], [280, me_msg.sign_it.blz, me_msg.sign_it.kunden_id, 'V', 0, 0], 0]) }
      me_msg.hnvsk.nr = 998
...
// sollte hier garnicht auftreten
        ...
        if (me_msg.segments[i].bez == 0) { body += "sendMsg.addSeg(Helper.newSegFromArray('" + me_msg.segments[i].name + "', " + me_msg.segments[i].vers + ', ' + JSON.stringify(seg_array) + '));\n\r' } else { body += "sendMsg.addSeg(Helper.newSegFromArrayWithBez('" + me_msg.segments[i].name + "', " + me_msg.segments[i].vers + ',' + me_msg.segments[i].bez + ',' + JSON.stringify(seg_array) + '));\n\r' }
      }
    }
    return top + sig + body
  }
