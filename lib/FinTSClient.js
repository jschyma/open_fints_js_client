...
// Logger
  me.log					= {
    'main': (logger || bunyan.createLogger({name: 'open-fin-ts-js-client', streams: []}))
  }
...
// 7. Analysiere Verfügbare Tan Verfahren
              try {
                ...
                for (var i = 0; i != HIRMS_for_tanv.store.data.length; i++) {
                  ...
                  if (me.upd.availible_tan_verfahren.length > 0) { me.log.gv.info({gv: 'HKVVB'}, 'Update to use Tan procedure: ' + me.upd.availible_tan_verfahren[0]) }
                  break
                }
                }
...
// ==> Version wird wohl nicht unterstützt, daher neu probieren mit HBCI2 Version
            me.log.conest.debug({step: step, hirms: HIRMS}, 'Version 300 nicht unterstützt, Switch Version from FinTS to HBCI2.2')
...
// Ensure the sequence of messages!
    ...
    var int_callback = function (param_1, param_2) {
      if (!in_finishing) { me.in_connection = false }
...
// Hier wird dann weiter gemacht :)
  ...
  me.debugLogMsg = function (txt, send) {
    ...
    if (me.debug_mode) { console.log((send ? 'Send: ' : 'Recv: ') + txt) }
  }
}
