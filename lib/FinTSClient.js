...
var FinTSClient = function (in_blz,in_kunden_id,in_pin,bankenlist,logger) {
    ...
    me.client_version 		= 1;
	me.proto_version		= 300;// 300=FinTS;220=Version 2.2
	...
	me.MsgEndDialog = function(cb){
		...
		me.SendMsgToDestination(msg,function(error,recvMsg){
			if(error){
				me.log.gv.log(error,{gv:"HKEND",msg:msg},"HKEND could not be send");
				cb(error,recvMsg);
			}else{
				cb(error,recvMsg);
			}
	...
	me.MsgRequestSepa = function(for_konto,cb){
		...
		me.SendMsgToDestination(msg,function(error,recvMsg){
			if(error){
				me.log.gv.log(error,{gv:"HKSPA",msg:msg},"HKSPA could not be send");
			    ...
			    try{
					...
					me.log.gv.log(ee,{gv:"HKSPA",msg:msg,resp_msg:recvMsg},"Exception while parsing HKSPA response");
	...
	*/
	me.MsgGetKontoUmsaetze = function(konto,from_date,to_date,cb){
		...
		me.SendMsgToDestination(msg,function(error,recvMsg){
			if(error){
				me.log.gv.log(error,{gv:"HKKAZ",msg:msg},"HKKAZ could not be send");
				cb(error,recvMsg,null);
			}else{
			    try{
					...
					if(hirms.getEl(1).getEl(1)=="0020"){
						...
						me.log.gv.error({gv:"HKKAZ",msg:msg,resp_msg:recvMsg},"We expected HIRMS 0020");
						cb("Fehlerrückmeldung: "+hirms.getEl(1).getEl(1)+" - "+hirms.getEl(1).getEl(3),recvMsg,null);
					}
			    }catch(ee){
					me.log.gv.log(ee,{gv:"HKKAZ",msg:msg,resp_msg:recvMsg},"Exception while parsing HKKAZ response");
			...
			me.MsgInitDialog(function(error,recvMsg,has_neu_url){
				if(error){
					if(vers_step==1){
						...
						me.log.conest.error({step:step,hirms:HIRMS},"Test Switch Version from FinTS to HBCI2.2 failed.");
					...
					me.log.conest.debug({step:step,bpd:me.bpd,upd:me.upd,url:me.bpd.url,new_sig_method:me.upd.availible_tan_verfahren[0]},"Init Dialog successful.");
					if(step==1||step==2){
		...
		var req = prot.request(options, function(res) {//https.request(options, function(res) {
		  ...
		  res.on('end', function(){
			...
			try{
				...
				me.log.con.log(e,"Could not parse received Message");
