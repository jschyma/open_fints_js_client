    ...
    me.SendMsgToDestination(msg, function (error, recvMsg) {
      if (error) {
        ...
        if (HIRMG != null && (HIRMG.getEl(1).getEl(1) == '0010' || HIRMG.getEl(1).getEl(1) == '3060')) {
          ...
          try {
            cb('Fehlerhafter Rückmeldungscode: ' + (HIRMG === null ? 'keiner' : HIRMG.store.data.filter(data => data.data[0].charAt(0) === '9').map(data => data.data[0] + ': ' + data.data[2]).join(', ')), recvMsg, false)
          } catch (cb_error) {
            me.log.gv.error(cb_error, {
              gv: 'HKVVB'
            }, 'Unhandled callback Error in HKVVB,HKIDN')
