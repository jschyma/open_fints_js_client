...
var FinTSClient = function (in_blz,in_kunden_id,in_pin,bankenlist) {
	...
	*/
    ...
    me.SendMsgToDestination = function(msg,callback){// Parameter für den Callback sind error,data
		...
		classes.log_con.debug({host:u.hostname,port:u.port,path:u.path},"Connect to Host");
		var req = prot.request(options, function(res) {//https.request(options, function(res) {
		  ...
		  res.on('end', function(){
			...
			classes.log_con.debug({host:u.hostname,port:u.port,path:u.path},"Request finished");
			...
			try{
				...
				classes.log_con.log(e,"Could not parse received Message");
		...
		req.on('error', function(){
			...
			classes.log_con.error({host:u.hostname,port:u.port,path:u.path},"Could not connect to "+options.hostname);
	...
	me.debugLogMsg = function(txt,send){
		classes.log_con.trace({raw_data:txt,send_or_recv:send?"send":"recv"},"Connection Data Trace");
		if(me.debug_mode)
